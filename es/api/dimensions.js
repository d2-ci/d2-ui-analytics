...
import { CHART_AGGREGATE_AGGREGATABLE_TYPES } from '../modules/dataTypes'; // Request util functions
...
const fetchIndicators = ({
  ...
  const order = `order=${nameProp}:asc`;
  let filter = groupId !== 'ALL' ? `&filter=indicatorGroups.id:eq:${groupId}` : '';
...
const fetchTrackedEntityAttributes = ({
  ...
  return request(d2, `programs/${groupId}`, paramString, {
    selectorFn: r => Array.isArray(r.programTrackedEntityAttributes) ? r.programTrackedEntityAttributes.map(a => a.trackedEntityAttribute).map(a => _objectSpread({}, a, {
      id: `${groupId}.${a.id}`,
      name: `${r.name} ${a.name}`
    })) : []
  });
...
const getEventDataItems = async (d2, queryParams) => {
  const [dataElementsObj, attributes] = await Promise.all([fetchProgramDataElements(d2, queryParams), fetchTrackedEntityAttributes(d2, queryParams)]);

  const filterInvalidTypes = item => Boolean(CHART_AGGREGATE_AGGREGATABLE_TYPES.includes(item.valueType));

  return _objectSpread({}, dataElementsObj, {
