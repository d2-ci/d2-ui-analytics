...
export const getOrgUnitsFromIds = (ids, metadata, parentGraphMap, idsToExclude = []) => {
  return ids.filter(id => !idsToExclude.includes(id)).filter(id => metadata[id] !== undefined).map(id => ({
    id: id,
...
export const getLevelsFromIds = (ids, levelOptions) => {
  ...
  return ids.filter(isLevelId).map(id => id.substr(LEVEL_ID_PREFIX.length + 1)).map(level => levelOptions.find(option => Number(option.level) === Number(level)).id);
};
/**
...
export const getGroupsFromIds = (ids, groupOptions) => {
  ...
  return ids.filter(isGroupId).map(id => id.substr(GROUP_ID_PREFIX.length + 1));
};
/**
...
export const transformOptionsIntoMetadata = (options, metadata, fields = ['id', 'displayName', 'name']) => {
  const result = {};
