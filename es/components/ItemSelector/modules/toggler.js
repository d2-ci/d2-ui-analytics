export const toggler = (id, isCtrlPressed, isShiftPressed, index, lastClickedIndex, highlightedIds, items) => {
  let ids;
...
const mergeIds = (highlightedIds, items, minIndex, maxIndex) => highlightedIds.length ? items.slice(minIndex, maxIndex + 1).filter(id => !highlightedIds.includes(id)).concat(highlightedIds) : items.slice(minIndex, maxIndex + 1);
